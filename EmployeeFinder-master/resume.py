...
@resume.route('/upload_file', methods=['GET','POST'])
def upload_file():
    ...
    return redirect(url_for('employee.upload_resume'))

@resume.route('/download/<code>', methods=['GET'])
def download(code):
    files = _get_files()
    if code in files:
...
@resume.route('/process',methods=["GET","POST"])
def process():
