...
@employee.route('/upload_resume',methods=['get','post'])
def upload_resume():
	return render_template("empresumeupload.html")

"""@employee.route('/empsendfeedback',methods=['get','post'])
def empsendfeedback():
	ids=session['login_id']
	if 'submit' in request.form:
		feed=request.form['feed']
		q="insert into feedback values(null,(select employee_id from employee where login_id='%s'),'%s','Pending',Curdate())"%(ids,feed)
		insert(q)
		flash("Feedback Added..")
		return redirect(url_for('employee.emphome'))
	return render_template('empsend_feedback.html')"""

@employee.route('/empsndfeedback',methods=['get','post'])
def empsndfeedback():
	ids=session['login_id']
	if 'submit' in request.form:
		feed=request.form['feed']
		q="insert into empfeedback values(null,(select employee_id from employee where login_id='%s'),'%s','Pending',Curdate())"%(ids,feed)
		insert(q)
		flash("Feedback Added..")
		return redirect(url_for('employee.emphome'))
	return render_template('empsend_feedback.html')

@employee.route('/viewfeedback',methods=['get','post'])
def viewfeedback():
	data={}
	ids=session['login_id']
	q="select *,emp_name as NAME from empfeedback inner join employee using(employee_id) where login_id='%s'"%(ids)
	res=select(q)
	data['feed']=res
	if 'action' in request.args:
		action=request.args['action']
		...
		action=None
	if action=="delete":
		q="delete from empfeedback where feedback_id='%s'"%(id)
		delete(q)
		flash("Feedback Deleted..")
		return redirect(url_for('employee.viewfeedback'))
	return render_template('empview_feedback.html',data=data)
