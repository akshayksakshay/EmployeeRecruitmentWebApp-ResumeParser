...
from resume import UPLOAD_FOLDER
import json
import os
...
def view_reference():
	data={}
	id=request.args['id']
	q="select * from reference inner join employee using(employee_id)where employee_id='%s'" % (id)
	res=select(q)
	data['ref']=res
...
@user.route('/viewemp_status',methods=['get','post'])
def viewemp_status():
	data={}
	ids=session['login_id']
	q="select *,concat(first_name,' ',last_name)as NAME from job_offer inner join user using(user_id) inner join skill using(skill_id) inner join employee using(employee_id) where user.login_id='%s'"%(ids)
	res=select(q)
	data['jo']=res
	return render_template('userview_empstatus.html',data=data)
...
@user.route('/resume_parser', methods=['GET','POST'])
def resume_parser():
    return render_template("userresumeparser.html")

@user.route('/view_employees',methods=['get','post'])
def view_employees():
	data={}
	q="select * from employee"
	res=select(q)
	...
	return render_template("userview_regemp.html",data=data)

@user.route('/view_fdback',methods=['get','post'])
def view_fdback():
	data={}
	q="select *,emp_name as NAME from empfeedback inner join employee using(employee_id)"
	res=select(q)
	data['feed']=res
	if 'action' in request.args:
		action=request.args['action']
	...
	if action=="delete":
		...
		return redirect(url_for('user.view_fdback'))
	j = 0
	for i in range(1, len(res) + 1):
		if 'submit' + str(i) in request.form:
			reply = request.form['reply' + str(i)]
			q = "UPDATE empfeedback SET reply='%s' WHERE feedback_id='%s'" % (reply, res[j]['feedback_id'])
			update(q)
			flash("Send Message...")
			return redirect(url_for('user.view_fdback'))
		j = j + 1
	return render_template("userview_fdback.html",data=data)